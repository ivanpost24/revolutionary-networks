...
from revnet_verify import progress, PROGRAM_DIR
from revnet_verify.geonames import (
    get_geonames,
...
the type of issue, and an explanatory message.  The program also ignores rows which do not have a location entry; these
are reported at the top of the log.  You can disable certain types of issues with the -d flag; use the "list" command
to get the issue types or the "describe" command to get the description of a specific issue.
